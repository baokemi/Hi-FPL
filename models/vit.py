...
def prompt_generator(model, x, args,sample_size=None):
        ...
        if model.prompt_sample_type == "random":
            ...
            for i in range(1,length):
                # 将x[i]的形状从(batch_size,length,dim)转换为(batch_size*length,dim)
                # x[i]:[batch_size,196,768]
                x_ = torch.reshape(x[i], shape=(-1, x[i].shape[-1]))
                # L = batch_size*196,D = 768
                L, D = x_.shape
                # noise是一个长度为196的一维向量
                noise = torch.rand(L, requires_grad=False)   # noise in [0, 1]
                # 对noise进行升序排列
                ids_shuffle = torch.argsort(noise, dim=0)
                # ids_keep[self.n_prompt]
                ids_keep = ids_shuffle[:model.n_prompt]
                # print("x_.device",x_.device)  # 确认 x_ 的设备
                
                ids_keep = ids_keep.to(x_.device)  # 确保 ids_keep 在同一个设备上
                # print("ids_keep.device",ids_keep.device)  # 确认 ids_keep 的设备
                # 经过unsqueeze(-1)后，ids_keep的形状为(self.n_prompt, 1)，
                # 再经过repeat函数形状变为（self.n_prompt,768）
                # torch.gather函数用于从x_中沿着第0维度收集元素，index为要收集的元素的索引
                prompt_ = torch.gather(x_, dim=0, index=ids_keep.unsqueeze(-1).repeat(1, D)).cpu().numpy()
                # prompt_: [self.n_prompt, 768]
                prompt.append(prompt_)
            prompt = torch.as_tensor(np.array(prompt))
            # prompt = prompt.unsqueeze(1).expand(-1,B,-1,-1)
            ...
            for i in range(1,length):
                ...
                ids_keep = ids_shuffle[:model.n_prompt]
                prompt_ = torch.gather(x_, dim=0, index=ids_keep.unsqueeze(-1).repeat(1, D)).cpu().numpy()
                # prompt_ = prompt_[np.newaxis, :, :]
                # prompt_ = np.repeat(prompt_,B,axis=0)
                prompt.append(prompt_)
                # prompt_.unsqueeze(0).expand(B,-1,-1)
            prompt = torch.as_tensor(np.array(prompt))
            print("prompt",prompt.shape)
...
class VisionTransformer_prompt(nn.Module):
    ...
    def __init__(self, config,img_size=224, num_classes=21843, vis=False, args = None):
        ...
        # 存储prompt的地方
        # self.prompt_common_store = [torch.zeros(self.n_prompt, config.hidden_size) for _ in range(len(self.share_blocks))]
        # self.prompt_uncommon_store = [torch.zeros(self.n_prompt, config.hidden_size) for _ in range(len(self.share_blocks_g))]
        # prompt层
        ...
        if self.pre_generate:
            ...
            for i,layer_block in enumerate(self.transformer.encoder.layer):
                if i in self.share_blocks:
                    ...
                    # prompt_tokens = self.prompt_common[self.share_blocks.index(i)].to(x.device)
                    # print("x.shape",x.shape)
                ...
                if i in self.share_blocks_g:
                    ...
                    # prompt_tokens = self.prompt_common[self.share_blocks_g.index(i)].to(x.device)
                    # print("x.shape",x.shape)
                    # print("prompt_tokens.shape",prompt_tokens.shape)
                    x=  torch.cat((
                                x[:, :self.args.n_prompt, :],
